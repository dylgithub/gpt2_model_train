...
def set_args():
    ...
    parser.add_argument('--model_path', default='model_save/min_ppl_model', type=str, required=False, help='对话模型路径')
...
def main():
    ...
    while True:
        try:
            ...
            # 是一个list记录一次对话包括历史输入和当次输入以及特殊标记在内的所有token_id
            ...
            for _ in range(args.max_len):
                ...
                # 获取每个位置的输出。如果第一次输入：讲个故事
                # 这里循环输出预测的下一个字[1,6,vocab_size]
                logits = outputs.logits
                # 获取最后一个生成的字[vocab_size]
