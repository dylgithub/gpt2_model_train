...
def set_args():
    ...
    #                     help='词表路径')
    parser.add_argument('--model_config', default='model/model_epoch40/config.json', type=str, required=False,
                        help='设置模型参数')
    ...
    parser.add_argument('--save_model_path', default='model_save', type=str, required=False,
                        help='模型输出路径')
    ...
    parser.add_argument('--warmup_steps', type=int, default=1000, help='warm up步数')
    # parser.add_argument('--label_smoothing', default=True, action='store_true', help='是否进行标签平滑')
    parser.add_argument('--val_num', type=int, default=1000, help='验证集大小')
    ...
    for epoch in range(args.epochs):
        ...
        if validate_loss < best_val_loss:
            ...
            if not os.path.exists(model_path):
                os.mkdir(model_path)
            model_to_save = model.module if hasattr(model, 'module') else model
            # 注意这种保存形式，保存之后再使用可以直接通过model.from_pretrained(model_path)的方式加载
            model_to_save.save_pretrained(model_path)
...
def main():
    ...
    if args.pretrained_model:  # 加载预训练模型
        print("预训练模型的路径：", args.pretrained_model)
